...
namespace Thomas.Database.Tests.Oracle
{
    public class OracleTests : IDatabaseProvider
    {
        ...
        public void Initialize()
        {
            ...
            DbConfigurationFactory.Register(new DbSettings("db1", SqlProvider.Oracle, ConnectionString));
        }
        ...
        public void GuidTest()
        {
            ...
            var data2 = dbContext.ToSingle<SimpleGuidRecord>($"SELECT :Value as Value FROM DUAL", param);
        ...
        public void TimeSpanTest()
        {
            ...
            var data2 = dbContext.ToSingle<SimpleTimeSpanRecord>($"SELECT :Value as Value FROM DUAL", param);
            Assert.That(param.Value, Is.EqualTo(data2.Value));

            var param2 = new { Value = "00:10:00" };
            var data3 = dbContext.ToSingle<SimpleTimeSpanRecord>($"SELECT :Value as Value FROM DUAL", param2);
            Assert.That(new TimeSpan(0, 10, 0), Is.EqualTo(data3.Value));

            Assert.Throws<TimeSpanConversionException>(() => dbContext.ToSingle<SimpleTimeSpanRecord>($"SELECT 'some string value' as Value FROM DUAL"));
        }
        ...
        public void GetUserStoreProcedure()
        {
            ...
            var userFromDb = dbContext.ToSingle<User>("GET_USER", new { id });
            var userRecordFromDb = dbContext.ToSingle<UserNullableRecord>("GET_USER", new { id });
            var userClassFromDb = dbContext.ToSingle<UserNullableClass>("GET_USER", new { id });
        ...
        public async Task GetUserStoreProcedureAsync()
        {
            ...
            var user = new User(0, 3, "Carlos", true, 1340.5m, new DateTime(1997, 3, 21), Guid.NewGuid(), icon);
            var id = dbContext.Add<User, int>(user);
            var userFromDb = await dbContext.ToSingleAsync<User>("GET_USER", new { id });
            var userRecordFromDb = await dbContext.ToSingleAsync<UserNullableRecord>("GET_USER", new { id });
            var userClassFromDb = await dbContext.ToSingleAsync<UserNullableClass>("GET_USER", new { id });
