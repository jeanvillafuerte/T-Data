...
namespace Thomas.Database.Core.FluentApi
{
    public class TableBuilder
    {
        ...
        public DbTable Configure<T>(Expression<Func<T, object>> expression, bool keyAutogenerated = true)
        {
            var memberExpression = EnsureSelectedMember(expression);
            var keyColumnName = memberExpression.Member.Name;
            var tableType = expression.Parameters[0].Type;
        ...
        {
            var properties = type.GetProperties().Where(x => string.IsNullOrEmpty(key) || x.Name != key);
