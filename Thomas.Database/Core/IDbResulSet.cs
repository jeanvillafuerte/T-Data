...
namespace Thomas.Database.Core
{
    public interface IDbResulSet
    {
        ...
        Task<Tuple<IEnumerable<T1>, IEnumerable<T2>>> ToTupleAsync<T1, T2>(string script, bool isStoreProcedure, CancellationToken cancellationToken) where T1 : class, new() where T2 : class, new();
        Task<Tuple<IEnumerable<T1>, IEnumerable<T2>, IEnumerable<T3>>> ToTupleAsync<T1, T2, T3>(string script, bool isStoreProcedure, CancellationToken cancellationToken) where T1 : class, new() where T2 : class, new() where T3 : class, new();
    }
}
