...
namespace Thomas.Database.Core.Provider
{
    internal static class DatabaseProvider
    {
        ...
        public static Action<object, DbCommand> GetCommandMetadata(in DbSettings options, in ulong key, in CommandType commandType, in Type type, in bool noCache, in bool excludeAutogeneratedColumns, in bool generateParameterWithKeys, in bool canCloseConnection, ref bool hasOutputParams, ref CommandBehavior commandBehavior, ref string[] filters)
        {
            if (type == null)
            {
                if (!noCache)
            ...
            Action<object, DbCommand>? loadParametersDelegate = DatabaseHelperProvider.GetLoadCommandParametersDelegate(in type, excludeAutogeneratedColumns, in generateParameterWithKeys, in options, ref hasOutputParams, ref filters);
