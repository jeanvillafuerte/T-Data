...
namespace Thomas.Database
{
    public interface IThomasDb
    {
        ...
        DataBaseOperationResult<T> ToSingle<T>(object searchTerm, string script) where T : class, new();

        DataBaseOperationResult<IReadOnlyList<T>> ToList<T>(string scriptRaw, bool isStoreProcedure = true) where T : class, new();
        DataBaseOperationResult<IReadOnlyList<T>> ToList<T>(object searchTerm, string procedureName) where T : class, new();
