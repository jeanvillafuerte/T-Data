...
namespace Thomas.Tests.Performance.Legacy.Tests
{
    internal class WriteOperations(string databaseName) : TestCase(databaseName)
    {
        ...
        {
            PerformOperation(() => DbHub.Use(db).Add(new Person { FirstName = "Jhon", LastName = "Doe", Country = "Peru", Salary = 9000, State = true, UserName = "JDoe" }), null, "Insert entity");
            PerformOperation(() => DbHub.Use(db).Add<Person, int>(new Person { FirstName = "Jean", LastName = "Villafuerte", Country = "Peru", Salary = 9000, State = true, UserName = "Jean" }), null, "Insert  entity return ID");
