...
namespace Thomas.Tests.Performance.Benchmark
{
    ...
    public class ThomasDataAdapterBenckmark : BenckmarkBase
    {
        ...
        {
            return Database.ToList<Person>($"SELECT * FROM {TableName} WHERE Id > @Id", new { Id = 0 });
        }

        //[Benchmark(Description = "ToList<> (buffered)")]
        //public void ToListCached()
        //{
        //    var list = Database2.ToList<Person>($"SELECT * FROM {TableName} WHERE Id > @Id", new { Id = 0 });
        ...
        {
            return Database.ToList<Person>(x => x.Id > 0);
        }
